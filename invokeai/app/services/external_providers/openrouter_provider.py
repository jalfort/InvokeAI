"""OpenRouter provider — text-only (prompt optimization via various models)."""
...
class OpenRouterProvider(BaseProvider):
    """OpenRouter text-only provider. Used for prompt optimization, not image generation."""
    ...
    async def generate(
        ...
        raise NotImplementedError("OpenRouter provider is text-only. Image generation is not supported.")
    ...
    async def edit(
        ...
        raise NotImplementedError("OpenRouter provider is text-only. Image editing is not supported.")
    ...
    async def test_key(self, api_key: str) -> tuple[bool, str]:
        """Validate an OpenRouter API key with a minimal chat completion call."""
        import requests
