...
export class CanvasSelectionToolModule extends CanvasModuleBase {
  ...
  /**
   * Builds a feathered selection mask from a signed distance field — the "buttery" way, matching
   * how GIMP/Krita/Photoshop actually feather: reconstruct a crisp sub-pixel edge, then
   * Gaussian-blur it. The blur IS the feather. A convolution cannot terrace or spike, and a wide
   * blur (sigma >> the ~1px staircase structure) obliterates the distance-field creases that used
   * to show as radial spokes. Pure function — sets no atoms.
   *
   * `radius` and the distance field share units (screen px for the live preview, document px for
   * the native commit mask), so sigma/offset track the caller's resolution automatically.
   *
   * Direction is chosen by WHERE the crisp edge sits (sdt is inside-positive):
   * - Both/center: edge on the outline (0)   — feather straddles it
   * - Inward:      edge pushed +R/2 inside   — feather lands inside the shape
   * - Outward:     edge pushed -R/2 outside  — feather lands outside the shape
   */
  private buildFeatheredCanvas(
    ...
    // Directional edge offset, in the same units as the signed distance field.
    ...
    // Step 1 — reconstruct a razor-thin (~1px), sub-pixel-accurate edge at the offset position.
    // A 1px linear ramp of the signed distance gives a clean anti-aliased step for the blur to
    // act on. No smoothstep-over-R here: the ramp comes entirely from the blur in step 2.
    const imageData = ctx.createImageData(width, height);
    const data = imageData.data;
    const size = width * height;
    for (let i = 0; i < size; i++) {
      const d = sdt[i]! - edgeOffset;
      let a = d + 0.5;
      a = a < 0 ? 0 : a > 1 ? 1 : a;
      const idx = i * 4;
      data[idx] = 255;
    ...
    // Step 2 — Gaussian-blur the crisp edge into the feather ramp. A blurred step's ~10-90% width
    // is ~2.56*sigma, so sigma ~= 0.33*R gives a visible feather roughly R wide. Uses the
    // browser-native (GPU-accelerated, genuinely Gaussian) canvas blur filter.
    const sigma = Math.max(0.5, radius * 0.33);
  ...
  /**
   * Union of all additive sub-selection bounds (stage-relative / document coords). Subtract
   * entries are excluded since they only shrink the selection. Returns null if empty.
   */
  private getSelectionBoundsUnion(): Rect | null {
    const stack = this.$subSelections.get();
    let minX = Infinity;
    let minY = Infinity;
    let maxX = -Infinity;
  ...
  /**
   * Target rect for a fill op = union(entity content, selection bounds) padded for the feather,
   * so the fill covers the whole selection even beyond current content (the layer grows, no hard
   * clip line). Stage-relative coords. Returns null if there is no selection.
   */
  private getOpTargetRect(contentRect: Rect): Rect | null {
    const selBounds = this.getSelectionBoundsUnion();
    if (!selBounds) {
      return null;
    }
    const scale = this.manager.stage.getScale();
    // 1.5x headroom: the Gaussian feather (buildFeatheredCanvas) extends the outward edge to
    // ~R/2 + ~2*sigma (sigma ~= 0.33R) ≈ 1.16R past the outline, so pad generously to avoid
    // clipping the soft tail at the op-mask edge.
    const pad = Math.ceil((this.manager.stateApi.getSettings().selectionFeatherRadius * 1.5) / scale) + 2;
    const minX = Math.floor(Math.min(contentRect.x, selBounds.x) - pad);
    const minY = Math.floor(Math.min(contentRect.y, selBounds.y) - pad);
    const maxX = Math.ceil(Math.max(contentRect.x + contentRect.width, selBounds.x + selBounds.width) + pad);
    const maxY = Math.ceil(Math.max(contentRect.y + contentRect.height, selBounds.y + selBounds.height) + pad);
    return { x: minX, y: minY, width: maxX - minX, height: maxY - minY };
  }

  /**
   * Rasterizes one sub-selection's shape at NATIVE (document) resolution into a canvas sized to
   * `targetRect`. Geometry is stored in stage-relative coords, so we draw it directly (no scale
   * multiply), offset by the target-rect origin. 1 canvas px = 1 document px.
   */
  private rasterizeEntryNative(entry: SubSelection, targetRect: Rect): HTMLCanvasElement {
    const canvas = document.createElement('canvas');
    canvas.width = Math.max(1, Math.round(targetRect.width));
    canvas.height = Math.max(1, Math.round(targetRect.height));
    const ctx = canvas.getContext('2d');
    if (!ctx) {
      return canvas;
    }
    const ox = targetRect.x;
    const oy = targetRect.y;
    ctx.fillStyle = 'white';
    if (entry.selectionMode === 'lasso') {
      // Freehand lasso: smooth the captured points into a curve (quadratic through edge midpoints)
      // so sparse mouse sampling doesn't leave visible straight-segment facets on the feathered edge.
      const pts = entry.lassoPoints;
      if (pts && pts.length >= 6) {
        const n = pts.length / 2;
        ctx.beginPath();
        const x0 = pts[(n - 1) * 2]! - ox;
        const y0 = pts[(n - 1) * 2 + 1]! - oy;
        const x1 = pts[0]! - ox;
        const y1 = pts[1]! - oy;
        ctx.moveTo((x0 + x1) / 2, (y0 + y1) / 2);
        for (let i = 0; i < n; i++) {
          const cx = pts[i * 2]! - ox;
          const cy = pts[i * 2 + 1]! - oy;
          const ni = (i + 1) % n;
          const nx = pts[ni * 2]! - ox;
          const ny = pts[ni * 2 + 1]! - oy;
          ctx.quadraticCurveTo(cx, cy, (cx + nx) / 2, (cy + ny) / 2);
        }
        ctx.closePath();
        ctx.fill('evenodd');
      }
    } else if (entry.selectionMode === 'polygon') {
      // Explicit polygon vertices — keep sharp corners (straight edges are intended here).
      const pts = entry.lassoPoints;
      if (pts && pts.length >= 6) {
        ctx.beginPath();
      ...
      const b = entry.bounds;
      ctx.beginPath();
      ctx.ellipse(b.x - ox + b.width / 2, b.y - oy + b.height / 2, b.width / 2, b.height / 2, 0, 0, Math.PI * 2);
      ctx.fill();
    } else {
      const b = entry.bounds;
      ctx.fillRect(b.x - ox, b.y - oy, b.width, b.height);
    }
    return canvas;
  }

  /**
   * Builds the combined, feathered selection mask at NATIVE resolution over `targetRect`
   * (stage-relative). Rebuilds each sub-selection from stored geometry (so quality is independent
   * of viewport zoom), combines them per combine mode, then feathers the combined boundary once
   * via the SDT worker — no internal seams. The screen-space feather radius is converted to
   * document px (÷ scale) so the committed feather matches the on-screen preview width.
   */
  private async buildNativeOpMask(targetRect: Rect): Promise<HTMLCanvasElement | null> {
    const stack = this.$subSelections.get();
    if (stack.length === 0) {
      return null;
    }
    const w = Math.max(1, Math.round(targetRect.width));
    const h = Math.max(1, Math.round(targetRect.height));
    ...
    for (const entry of stack) {
      const entryCanvas = this.rasterizeEntryNative(entry, targetRect);
      bctx.globalCompositeOperation = entry.combineMode === 'subtract' ? 'destination-out' : 'source-over';
      bctx.drawImage(entryCanvas, 0, 0);
    }
    ...
    const scale = this.manager.stage.getScale();
    const radiusNative = settings.selectionFeatherRadius / scale;
    const imageData = bctx.getImageData(0, 0, w, h);
    const buffer = imageData.data.buffer.slice(0);
    const sdt = await new Promise<Float32Array>((resolve) => {
      this.manager.worker.requestSdt({ buffer, width: w, height: h }, (sdtArr) => resolve(sdtArr));
    });
    return this.buildFeatheredCanvas(sdt, w, h, radiusNative, settings.selectionFeatherDirection);
  }

  /**
   * Fills the selected area with the current brush color.
   * Rebuilds the feathered selection mask at the layer's native resolution (crisp at any zoom)
   * over the union of content + selection bounds (no hard clip line), then composites + replaces.
   */
  fillSelection = async (): Promise<void> => {
    ...
    try {
      ...
      const contentRect = selectedEntity.transformer.getRelativeRect();

      // Target rect = union(entity content, selection bounds) padded for the feather, so the fill
      // covers the whole selection even where it extends past current content — the layer grows and
      // there is no hard clip line. Stage-relative coords (1px = 1 document px).
      const targetRect = this.getOpTargetRect(contentRect);
      if (!targetRect) {
        return;
      }

      // Native-resolution feathered selection mask over targetRect (crisp regardless of zoom).
      const opMask = await this.buildNativeOpMask(targetRect);
      if (!opMask) {
        return;
      }

      // Snapshot the entity's current pixels WITHOUT dispatching to Redux (keeps the fill correct in
      // undo history — see deleteSelection for the throttled-no-op-rasterize rationale).
      const existingBlob = await selectedEntity.renderer.getBlob({ rect: contentRect });
      ...
      // Composite = existing content (placed at its offset within targetRect) + color through the mask.
      const composite = document.createElement('canvas');
      composite.width = Math.max(1, Math.round(targetRect.width));
      composite.height = Math.max(1, Math.round(targetRect.height));
      ...
      ctx.drawImage(
        existingImg,
        Math.round(contentRect.x - targetRect.x),
        Math.round(contentRect.y - targetRect.y),
        Math.max(1, Math.round(contentRect.width)),
        Math.max(1, Math.round(contentRect.height))
      );
      const colorMask = this.createFilledMask(opMask, color);
      if (!colorMask) {
        return;
      }
      ctx.drawImage(colorMask, 0, 0);

      // Upload + replace the entity with the composite at the target-rect position. We use
      // rasterizeEntity (not a plain commitBuffer) because 6.13's buffer commit switch has no
      // 'image' case — an image buffer would render transiently then drop on the next state sync.
      ...
      this.manager.stateApi.rasterizeEntity({
        ...
        imageObject,
        position: { x: Math.round(targetRect.x), y: Math.round(targetRect.y) },
        replaceObjects: true,
      });
  ...
  deleteSelection = async (): Promise<void> => {
    ...
    try {
      ...
      // Native-resolution feathered selection mask over the entity content rect (crisp at any zoom).
      // Erasing outside current content is a no-op, so the content rect is sufficient here.
      const opMask = await this.buildNativeOpMask(contentRect);
      if (!opMask) {
        return;
      }

      // Capture existing content directly WITHOUT dispatching to Redux. Using
      // rasterize({replaceObjects: false}) would dispatch a no-op entityRasterized action that gets
      // throttled with the real replacement (1s window), excluding the delete from undo history.
      const existingBlob = await selectedEntity.renderer.getBlob({ rect: contentRect });
      const existingImageDTO = await uploadImage({
        file: new File([existingBlob], 'selection_snapshot.png', { type: 'image/png' }),
        image_category: 'other',
      ...
      const composite = document.createElement('canvas');
      composite.width = Math.max(1, Math.round(contentRect.width));
      composite.height = Math.max(1, Math.round(contentRect.height));
      const ctx = composite.getContext('2d');
      if (!ctx) {
        return;
      }
      ...
      // Erase the selection region (native-res mask is 1:1 with the composite).
      ctx.globalCompositeOperation = 'destination-out';
      ctx.drawImage(opMask, 0, 0);
      ...
      this.manager.stateApi.rasterizeEntity({
        entityIdentifier: selectedEntity.entityIdentifier,
        imageObject,
        position: { x: Math.round(contentRect.x), y: Math.round(contentRect.y) },
        replaceObjects: true,
      });
