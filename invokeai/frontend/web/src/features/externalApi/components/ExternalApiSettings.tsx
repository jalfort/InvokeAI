...
export const ExternalApiSettings = memo(() => {
  ...
  return (
    <Flex flexDir="column" gap={3} pb={3}>
      ...
      {/* --- Schema-driven settings (both hardcoded and dynamic models with schemas) --- */}
      {selectedModelEntry?.cached_schema && (
        <>
          <FormControl>
      ...
      {/* --- Fallback: generic settings for models without schemas --- */}
      {!selectedModelEntry?.cached_schema && (
        <>
          <FormControl>
            <FormLabel>{t('externalApi.mode')}</FormLabel>
